...
const VehicleGridWithInfiniteLoading: React.FC<Props> = ({
  country = "ae",
  state,
  category = "cars",
  vehicleType,
  ...
  useEffect(() => {
    // Function to check window size and update mountMap
    const handleResize = () => {
      if (window.innerWidth >= 1024 && mountMap) {
        setMountMap(false); // Set mountMap to false when window is resized to 'lg' or above
      }
    };
...
export default VehicleGridWithInfiniteLoading;
