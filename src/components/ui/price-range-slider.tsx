      ...
      {initialValue.map((value, index) => (
        <React.Fragment key={index}>
          <SliderPrimitive.Thumb
            className={`relative block h-8 w-8 rounded-full border-2 bg-white ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:border-gray-400 disabled:opacity-50 ${disabled ? "cursor-default border-gray-200" : "cursor-grab border-yellow"}`}
            aria-label={`${index === 0 ? "Minimum" : "Maximum"} price: ${value}`}
          >
            {label && (
              <span
                className={cn(
